# OpenMM-ML with ANI-1nxr

## Installation 

Create a conda environment with `openmm`, `openmm-torch`, and `nnpops` and activate it:

```bash
conda create --name ani-env openmm openmm-torch nnpops -c conda-forge
```

Activate the environment:

```
conda activate ani-env
```

Install `torchani` from my fork:

```bash
git clone https://github.com/JMorado/torchani.git torchani-repo
cd torchani-repo
pip install -e .
cd ..
```

Install `openmm-ml` from my fork:

```bash
git clone https://github.com/jmorado/openmm-ml openmm-ml-repo
cd openmm-ml-repo
git checkout feature_1xnr
pip install -e .
cd ..
```
...
Example script for running an MD simulation:
